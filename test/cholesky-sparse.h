...
double sparseCholeskyWithReordering(const __core__::MatrixCXS<double,int,__core__::cpuAllocator,__core__::CCS>& AF,std::string outName="./cholr") {
 	using namespace std;
	using namespace __core__;
	using namespace __third_party__;
...
double sparseCholeskyWithReorderingTT(const __core__::MatrixCXS<double,int,__core__::cpuAllocator,__core__::CCS>& AF,std::string outName="./cholr") {
 	using namespace std;
	using namespace __core__;
	using namespace __third_party__;
	MatrixCXS<double,int,cpuAllocator,CCS> A,L,AT,AMetis;
	...
	cpu_timer timer;
	timer.start();
	choleskyLeftLooking(LH,AH,RPH,tree,ph,ch);
	timer.stop();
	cerr<<endl<<"Tree traversal sparse with reordering:"<<endl<<"\tElapsed time: "<<timer<<endl<<endl;
	std::ofstream Aout=std::move(open_file<1>(outName+"A.csv"));
	std::ofstream Lout=std::move(open_file<1>(outName+"L.csv"));
	std::ofstream Tout=std::move(open_file<1>(outName+"Tree.csv"));
	Aout << std::setprecision(12);
