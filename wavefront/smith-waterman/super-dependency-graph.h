...
std::ostream& operator<<(std::ostream& ost,const SmithWatermanVertex& v) {
	print(v.pos,ost)<<" ";print(v.dims,ost);
	return ost;
}
std::ostream& operator<<(std::ostream& ost,const SmithWatermanVertexDebug& v) {
	print(v.pos,ost)<<" ";print(v.dims,ost)<<" "<<v.etime<<" "<<v.tid;
	return ost;
}
GraphCXS<SmithWatermanVertex,float,cpuAllocator> smithWatermanSDG(size_t n,size_t m,size_t px,size_t py){
...
GraphCXS<SmithWatermanVertexDebug,float,cpuAllocator> smithWatermanSDGD(size_t n,size_t m,size_t px,size_t py){
	size_t nv=__ceil__(static_cast<double>(n)/px), mv=__ceil__(static_cast<double>(m)/py);
