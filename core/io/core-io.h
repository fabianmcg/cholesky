...
template <typename T,typename U=T,int n=0,uint alignment=0> std::ostream& print(const Vector<T,n,alignment>& v,std::ostream &ost=std::cerr,std::string separator=", ",std::string begin="{",std::string end="}") {
	ost<<begin;
