#ifndef __MEMORY_COPY_H__
#define __MEMORY_COPY_H__
...
//*************************************************** unified memory ***************************************************
//linear memory
template <SyncBehaviorType sync_behavior=ASYNC,typename T=void> void __memcpy__(void *dst_ptr,void *src_ptr,std::size_t size,cudaStream_t stream=DEFAULT_STREAM) {
...
//2D memory
template <SyncBehaviorType sync_behavior=ASYNC> void __memcpy__(void *dst_ptr,std::size_t dpitch,void *src_ptr,std::size_t spitch,std::size_t xsize,std::size_t ysize,cudaStream_t stream=DEFAULT_STREAM) {
	if(sync_behavior==ASYNC) {
...
//3D memory
template <SyncBehaviorType sync_behavior=ASYNC> void __memcpy__(cudaMemcpy3DParms &params,cudaStream_t stream=DEFAULT_STREAM) {
...
void __memcpy__(T *dst_ptr,T *src_ptr,std::size_t size,int device=0,cudaStream_t stream=DEFAULT_STREAM) {
	constexpr std::size_t sod=__memory_private__::__sizeof__<T>();
...
std::future<void*> __memcpy__(T *dst_ptr,T *src_ptr,std::size_t size,int device=0,cudaStream_t stream=DEFAULT_STREAM) {
	constexpr std::size_t sod=__memory_private__::__sizeof__<T>();
...
void __memcpy__(T *dst_ptr,T *src_ptr,std::size_t size,int device=-1,cudaStream_t stream=DEFAULT_STREAM) {
	constexpr std::size_t sod=__memory_private__::__sizeof__<T>();
...
void __memcpy__(T *dst_ptr,T *src_ptr,std::size_t size,int dst_device,int src_device,cudaStream_t stream=DEFAULT_STREAM) {
	constexpr std::size_t sod=__memory_private__::__sizeof__<T>();
	if(dst_device>=0) {
...
//2D memory
template <typename memory_DT,typename memory_ST,SyncBehaviorType sync_behavior=ASYNC> void __memcpy__(void *dst_ptr,std::size_t dpitch,void *src_ptr,std::size_t spitch,std::size_t xsize,std::size_t ysize,int device=-1,cudaStream_t stream=DEFAULT_STREAM) {
	constexpr cudaMemcpyKind mcpykind=__memory_private__::memcpykind<memory_ST,memory_DT>();
	if(device>=0) {
		cuda_error(cudaSetDevice(device),API_ERROR);
	}
...
void __memcpy__(cudaMemcpy3DParms &params,int device=-1,cudaStream_t stream=DEFAULT_STREAM) {
	if(device>=0) {
		cuda_error(cudaSetDevice(device),API_ERROR);
	}
...
template <typename memory_DT,typename memory_ST,SyncBehaviorType sync_behavior=ASYNC>
void __memcpy__(cudaMemcpy3DParms &params,int dst_device,int src_device,cudaStream_t stream=DEFAULT_STREAM) {
	if(dst_device>=0) {
		cuda_error(cudaSetDevice(dst_device),API_ERROR);
	}
...
template <SyncBehaviorType sync_behavior=ASYNC> void __memcpy__(cudaMemcpy3DPeerParms &params,cudaStream_t stream=DEFAULT_STREAM) {
...
template <typename memory_DT,typename memory_ST,SyncBehaviorType sync_behavior=ASYNC>
void __memcpy__(cudaPitchedPtr &dst_ptr,const cudaPitchedPtr &src_ptr,const cudaExtent &extent,int device=-1,cudaStream_t stream=DEFAULT_STREAM) {
...
template <typename memory_DT,typename memory_ST,SyncBehaviorType sync_behavior=ASYNC>
void __memcpy__(cudaPitchedPtr &dst_ptr,const cudaPitchedPtr &src_ptr,const cudaExtent &extent,const cudaPos &dst_pos,const cudaPos &src_pos,int device=-1,cudaStream_t stream=DEFAULT_STREAM) {
	cudaMemcpy3DParms params={0};
	params.dstPtr=dst_ptr;
...
template <typename memory_DT,typename memory_ST,SyncBehaviorType sync_behavior=ASYNC>
void __memcpy__(cudaPitchedPtr &dst_ptr,const cudaPitchedPtr &src_ptr,const cudaExtent &extent,int dst_dev,int src_dev,cudaStream_t stream=DEFAULT_STREAM) {
	cudaMemcpy3DParms params={0};
	params.dstPtr=dst_ptr;
	params.srcPtr=src_ptr;
	params.extent=extent;
...
template <typename memory_DT,typename memory_ST,SyncBehaviorType sync_behavior=ASYNC>
void __memcpy__(cudaPitchedPtr &dst_ptr,const cudaPitchedPtr &src_ptr,const cudaExtent &extent,const cudaPos &dst_pos,const cudaPos &src_pos,int dst_dev,int src_dev,cudaStream_t stream=DEFAULT_STREAM) {
	cudaMemcpy3DParms params={0};
	params.dstPtr=dst_ptr;
	params.dstPos=dst_pos;
	params.srcPtr=src_ptr;
