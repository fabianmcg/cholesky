...
//				for(size_t j=k+1;j<R.n();++j)
//					for(size_t i=j;i<R.n();++i) {
//#pragma omp task depend(in:R(k,i)) depend(in:R(k,j)) depend(in:R(k,k)) depend(out: R(j,i))
//						R(j,i)-=R(k,i)*R(k,j)/R(k,k);
//					}
//				for(size_t j=k+1;j<R.n();++j)
//#pragma omp task depend(out:R(k,j)) depend(in:R(k,k))
//					R(k,j)=R(k,j)/__sqrt__(__abs__(R(k,k)));
//#pragma omp task depend(out:R(k,k))
